...
#here is the logic for the gpt-4o model using Instructor and validators
    ...
    class SupplierFinancialDetails(BaseModel):
        ...
        observation: Optional[str] = Field(
            ...
            examples=[
                ...
                """
                "Invoice Review: The invoice mentions two suppliers, Sligro Food Group Nederland BV and Heineken Nederland BV.\n"
                "Only Sligro Food Group Nederland BV is actually billed, so we will ignore Heineken Nederland BV.\n"
                "The issuer of the invoice is 'Heineken Sligro Stichting Derdengelden'.\n"
                "Explicitly mentioned: Total Amount Payable: €1500.00\n"
                "Subtotaal exclusief BTW en emballage Sligro Food Group Nederland B.V. €1350.00\n"
                "Subtotaal BTW €300.00\n"
                "Sligro Food Group Nederland B.V. 9% over €300.00 is €27.00\n"
                "Sligro Food Group Nederland B.V. 21% over €800.00 is €168.00\n"
                "Subtotaal emballage is the relevant metric for packaging deposits, it's -€150.00\n"
                "Emballage does NOT qualify as 'null_tax_base', 'low_tax_base' or 'high_tax'.\n"
                "Conclusion: Given the complexity I should be careful and precise in my analysis.\n"
                "The amount payable includes the VAT tax and subtotaal emballage.\n"
                "Emballage is not subject to VAT (BTW). It is treated as VAT-exempt. Meaning neither count for 'null_tax_base', 'low_tax_base' or 'high_tax'.\n"
                "I will take extreme caution given the complexity of the invoice, and process the data correctly per individual supplier.\n"
...
"""
    ...
    class InvoiceDetail(BaseModel):
        ...
        discount: Optional[Discount] = Field(
            default=None,
            description="The details of the discount, credit or deduction that was applied to the amount payable."
        )
        ...
        def validate_tax_bases_and_taxes(self) -> Self:
            """Validate that the tax bases and taxes match the sum of the respective amounts for all suppliers."""
            ...
            if self.null_tax_base is not None and abs(self.null_tax_base - total_null_tax_base) > tolerance:
                raise ValueError(
                    f"#####START_VALUE_ERROR_MESSAGE#####\n"
                    f"value_error_id: 'null_tax_base_mismatch'\n"
                    f"Luca! Pay Attention! null_tax_base must equal the sum of the null_tax_base amounts for each supplier. Expected {total_null_tax_base}, got {self.null_tax_base}."
                    f"#####END_VALUE_ERROR_MESSAGE#####\n"
                    f"ValueError (ID) was raised, you must set 'flag_value_error' to TRUE and provide the exact error message, and an analysis of the error."
                )

            if self.low_tax_base is not None and abs(self.low_tax_base - total_low_tax_base) > tolerance:
                raise ValueError(
                    f"#####START_VALUE_ERROR_MESSAGE#####\n"
                    f"value_error_id: 'low_tax_base_mismatch'\n"
                    f"Luca! Pay Attention! low_tax_base must equal the sum of the low_tax_base amounts for each supplier. Expected {total_low_tax_base}, got {self.low_tax_base}."
                    f"#####END_VALUE_ERROR_MESSAGE#####\n"
                    f"ValueError (ID) was raised, you must set 'flag_value_error' to TRUE and provide the exact error message, and an analysis of the error."
                )

            if self.high_tax_base is not None and abs(self.high_tax_base - total_high_tax_base) > tolerance:
                raise ValueError(
                    f"#####START_VALUE_ERROR_MESSAGE#####\n"
                    f"value_error_id: 'high_tax_base_mismatch'\n"
                    f"Luca! Pay Attention! high_tax_base must equal the sum of the high_tax_base amounts for each supplier. Expected {total_high_tax_base}, got {self.high_tax_base}."
                    f"#####END_VALUE_ERROR_MESSAGE#####\n"
                    f"ValueError (ID) was raised, you must set 'flag_value_error' to TRUE and provide the exact error message, and an analysis of the error."
                )

            if self.low_tax is not None and abs(self.low_tax - total_low_tax) > tolerance:
                raise ValueError(
                    f"#####START_VALUE_ERROR_MESSAGE#####\n"
                    f"value_error_id: 'low_tax_mismatch'\n"
                    f"Luca! Pay Attention! low_tax must equal the sum of the low_tax amounts for each supplier. Expected {total_low_tax}, got {self.low_tax}. First check if the values are placed in the correct field, sometimes suppliers make mistakes."
                    f"#####END_VALUE_ERROR_MESSAGE#####\n"
                    f"ValueError (ID) was raised, you must set 'flag_value_error' to TRUE and provide the exact error message, and an analysis of the error."
                )

            if self.high_tax is not None and abs(self.high_tax - total_high_tax) > tolerance:
                raise ValueError(
                    f"#####START_VALUE_ERROR_MESSAGE#####\n"
                    f"value_error_id: 'high_tax_mismatch'\n"
                    f"Luca! Pay Attention! high_tax must equal the sum of the high_tax amounts for each supplier. Expected {total_high_tax}, got {self.high_tax}."
                    f"#####END_VALUE_ERROR_MESSAGE#####\n"
                    f"ValueError (ID) was raised, you must set 'flag_value_error' to TRUE and provide the exact error message, and an analysis of the error."
                )
    ...
    async with sem:
        ...
        try:
            ...
            # Move the erroneous file to the error directory
            error_dir = "./scripts/data/error"
            os.makedirs(error_dir, exist_ok=True)
            os.rename(file, os.path.join(error_dir, os.path.basename(file)))
            raise ValueError(f"Max retries exceeded for file {file}")
...
async def extract_receipt(file: UploadFile = File(...)):
    try:
        ...
        # Process with GPT-4
        result = await process_text_with_gpt4(text_content)
