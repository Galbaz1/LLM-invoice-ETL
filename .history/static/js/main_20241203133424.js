document.addEventListener('DOMContentLoaded', function() {
    ...
    function convertToMarkdown(data) {
        ...
        markdown += `Amount Payable: EUR ${data.amount_payable}\n\n`;
        ...
        markdown += `Supplier: ${data.primary_supplier}\n`;
        ...
        if (data.suppliers && data.suppliers.length > 0) {
            data.suppliers.forEach((supplier, index) => {
                if (supplier.high_tax_base) markdown += `High Tax Base: EUR ${supplier.high_tax_base}\n`;
                if (supplier.high_tax) markdown += `High Tax Amount: EUR ${supplier.high_tax}\n`;
                if (supplier.low_tax_base) markdown += `Low Tax Base: EUR ${supplier.low_tax_base}\n`;
                if (supplier.low_tax) markdown += `Low Tax Amount: EUR ${supplier.low_tax}\n`;
                if (supplier.null_tax_base) markdown += `Null Tax Base: EUR ${supplier.null_tax_base}\n`;
                if (supplier.amount_excl_tax) markdown += `Amount Excluding Tax: EUR ${supplier.amount_excl_tax}\n`;
            });
        }

        // Additional details
        if (data.total_emballage) markdown += `Total Emballage: EUR ${data.total_emballage}\n`;
        if (data.discount) markdown += `Discount: EUR ${data.discount}\n`;
