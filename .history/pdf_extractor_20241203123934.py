...
It saves the extracted text in a structured format.
...
# Disable duplicate logs from the library
logging.getLogger('unstract.llmwhisperer.client_v2').propagate = False
...
def save_results(result: Dict, original_pdf_path: str, output_dir: Path) -> None:
    """Save extraction results to text file."""
    try:
...
def main():
    ...
    try:
        ...
        print(colored(f"✓ All operations completed in {elapsed_time:.1f} seconds!", "green"))
